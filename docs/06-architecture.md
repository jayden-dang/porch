...
| 5 | **deliver** | Format leftover? Commit leftover with a porch subject. `ls-remote`, lease-push exact SHA, `gh pr create/update`, watch **allowlisted** checks, rebase+restart at **review** on conflict/repair. | 3 for mechanical check fails; never for deploy |
