...
| DISPO | Disposition history | Append-only porch-owned authority events on finding instances, bulk review operator-responses, and the dedicated audit document's disposition slice | disposition, authority event, bulk operator response, fix_requested, related_occurrences, audit document | `crates/porch-gate/src/`, `crates/porch-run/src/`, `crates/porch/src/` | `../2026-09-06-disposition-history/` | In-progress | ROAD-4 | — |
