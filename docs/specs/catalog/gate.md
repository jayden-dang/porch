...
| PHASE | Phase-event history | Append-only `phase_events` as the source of truth for a run's phase lifecycle: top-level attempts and ordinals, nested operations, atomic handoffs, crash evidence, derived repair budget, and the audit document's phase slice rendered by `porch audit` | phase event, phase attempt, nested operation, phase handoff, timeline, co-write seam | `crates/porch-gate/src/`, `crates/porch-run/src/`, `crates/porch/src/` | `../2026-09-07-phase-events/` | In-progress | ROAD-5 | — |
