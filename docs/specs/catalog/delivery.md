...
| FWDAUTH | Durable forward authorization | Binds authorization to exactly one reviewed commit and records a forward intent before the push and a forward outcome after it, so a crash mid-forward leaves durable evidence instead of an ambiguous run | forward authorization, forward record, head continuity, authorized sha, intent, outcome | `crates/porch-run/src/deliver.rs`, `crates/porch-gate/src/rounds/`, `crates/porch-run/src/lib.rs` | `../2026-09-08-forward-authorization/` | Draft | ROAD-7 | — |
