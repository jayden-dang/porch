...
| FWDAUTH | Durable forward authorization | Records a forward intent before the push and a forward outcome after it, so a crash mid-forward leaves durable evidence instead of a run that looks like it never tried; the forward carries the SHA continuity authorized, and a deliver attempt may record a sequence of forwards with at most one in flight. Binding that SHA by equality is blocked on the certify correction-commit question | forward authorization, forward record, head continuity, authorized sha, intent, outcome, forward sequence | `crates/porch-run/src/deliver.rs`, `crates/porch-gate/src/rounds/`, `crates/porch-run/src/lib.rs` | `../2026-09-08-forward-authorization/` | Implemented | ROAD-7 | — |
