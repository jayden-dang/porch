...
- [ ] **1.1** Remove `maybe_correction_commit` and `refresh_head_sha` from
  ...
  EQUAL-1.4)*
- [ ] **1.2** Make `porch-fake-format-dirty` a stable overwrite so a rebase-then-certify
  re-run is a no-op. Add `porch-fake-lint-dirty`. *(EQUAL-4.3, EQUAL-4.4)*
- [ ] **1.3** `lint_dirty_tree_fails_certify_and_does_not_commit`: lint that writes
  fails certify; log has no `porch: apply lint`. *(EQUAL-4.4)*

## Wave 2 — format at the end of rebase

- [ ] **2.1** Keep `cfg.commands` in `run_rebase`. After git rebase / reset /
...
- [ ] **2.2** Same helper on the parked-rebase fixer retry, from the pinned
  `trusted_config_sha`. Not on deliver-repair rebase. *(EQUAL-2.6, EQUAL-2.7)*
- [ ] **2.3** Retarget `format_dirty_tree_gets_correction_commit` and
...
- [ ] **3.1** `authorized_forward_sha`: drop `is_ancestor`; require `head ==
  approved`; return the approved SHA. Invert
  `head_continuity_fails_if_approved_sha_missing_on_certify`'s sibling that
  currently accepts a descendant. *(EQUAL-3.1 … 3.4)*
- [ ] **3.2** Convert `tripwire_a_correction_commit_forwards_an_unreviewed_sha`
  into the two-guard form. *(EQUAL-4.1)*
- [ ] **3.3** Confirm empty-diff skip still does not require an approved SHA when
  format is absent or a no-op. *(EQUAL-3.6)*

## Wave 4 — operator-facing words and the ledger

- [ ] **4.1** `docs/usage.md`: format runs at rebase and again as a check in
  certify; checking form is a supported choice. Pipeline table. *(EQUAL-5.1,
  EQUAL-5.2)*
- [ ] **4.2** `CONTEXT.md` **Certify** and **Forward authorization**. *(EQUAL-5.3)*
- [ ] **4.3** Mark FWDAUTH-1.1, 1.2, 1.3, 1.4, 1.6 completed by EQUAL. Register
  EQUAL in `docs/specs/catalog/delivery.md`. Close ROAD-23 on the roadmap; MILE-3
  can close if GOAL-1's four properties all hold.
- [ ] **4.4** FAULT's tripwire section records that the pin became a guard.
