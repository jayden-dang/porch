...
**Depends-on:** none
**Steps:**
- [x] Test: opening an old database applies the new tables and leaves existing rows readable.
- [x] Test: `open_round` returns an id only after commit; a second open allocates ordinal 2.
- [x] Test: a blob whose stored bytes disagree with its digest is refused.
- [x] Implement the seven tables, CHECKs, indexes, and the `review_history_revision` column.
- [x] Implement `open_round` under `BEGIN IMMEDIATE` with ordinal allocation.
- [x] Run `cargo test --workspace`; expect pass. Commit.
...
**Depends-on:** Task 1
**Steps:**
- [x] Test: an absent element and a present-but-empty element record different source states.
- [x] Test: an element over the 256 KiB ceiling stores its digest with snapshot omitted, source unchanged.
- [x] Test: an element not supplied to a layer records `not_applied` with no effective digest.
- [x] Implement element and application persistence with content-addressed snapshots.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** none
**Steps:**
- [x] Test: resolution happens once; the spawn uses the recorded absolute target and argv.
- [x] Test: a wrapper's identity spans wrapper, backend, and argv — not the wrapper digest alone.
- [x] Test: an unobservable version records `unavailable` with a reason, never a substitute.
- [x] Test: an opaque entrypoint records that only the entrypoint was observed.
- [x] Implement `prepare`, the descriptor, composite identity, and the post-spawn stability check.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** none
**Steps:**
- [x] Test: a quality finding's `rule_id` reaches the output and maps to a canonical criterion.
- [x] Test: a producer key is retained as provenance and never becomes the identity.
- [x] Test: a deterministic producer's finding carries no model-style confidence.
- [x] Test: scope-extending findings keep their forced `ask-user` action.
- [x] Implement `rule_id` exposure, criterion mapping, anchor fallback, and the candidate key.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** none
**Steps:**
- [x] Test: a path present in output without a completion signal is not `completed`.
- [x] Test: `failed` and `waived` carry reasons; `waived` carries authority; `completed` carries evidence.
- [x] Test: a changed file missing without a skip still fails the review closed.
- [x] Implement state derivation over the producer manifest.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** Task 4
**Steps:**
- [x] Write the seven fixture families with expectations as mappings, plus `MANIFEST.json`.
- [x] Test: moved code and a rewritten message reuse the prior fingerprint.
- [x] Test: two distinct issues sharing a candidate key receive different fingerprints.
- [x] Test: multi-producer duplicates collapse only on a common non-empty range intersection.
- [x] Test: ambiguity mints; a prior instance nothing claims simply disappears.
- [x] Implement matching, minting with the instance disambiguator, and the version-boundary rule.
- [x] Run tests; expect pass. Commit.
...
**Steps:**
- [x] Test: coverage, instances, and terminal state land in one transaction or not at all.
- [x] Test: a revision changed between phases yields `Stale` and no durable finalization.
- [x] Test: each instance gets a distinct id; instances sharing a fingerprint stay separate rows.
- [x] Test: a contention-free finalization commits exactly two writes beyond the pre-ROUND path.
- [x] Implement two-phase finalization, revision increment, and the three-retry bound.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** Task 7
**Steps:**
- [x] Test: a pending, incomplete, interrupted, or under-covered round never authorizes.
- [x] Test: differing only in `selection_source` or `declared_engine_kind` stays applicable.
- [x] Test: an unavailable producer version never establishes equivalence.
- [x] Test: a floor-plus-judgment round is not equivalent to a judgment-only round.
- [x] Implement the applicability tuple and multiset producer-set correspondence.
- [x] Run tests; expect pass. Commit.
...
**Steps:**
- [x] Test: a failed round open aborts before any producer is spawned.
- [x] Test: timeout, unsuccessful exit, malformed output, and coverage shortfall each finalize
      `finished`/`incomplete` with distinct reasons; a clean run finalizes `complete`.
- [x] Test: blocking findings still park the run and still finalize `complete`.
- [x] Test: two rounds of one run keep separate artifacts under their invocation namespaces.
- [x] Test: approve records the head SHA; skip leaves it unrecorded; post-fix `from_sha` is unchanged.
- [x] Implement the sequence, terminal-state mapping, and per-invocation artifact paths.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** Task 7
**Steps:**
- [x] Test: a process killed at each boundary leaves a round restart reconciles to
      `interrupted`/`incomplete`, with no instances and no approval.
- [x] Test: reconciliation uses at most one committed write per stale round.
- [x] Test: startup still recovers stale runs and still refuses to serve when recovery fails.
- [x] Implement stale-round reconciliation behind the existing recovery contract.
- [x] Run tests; expect pass. Commit.
...
**Depends-on:** Task 1
**Steps:**
- [x] Test: opening a round pins its trusted commit; the ref survives a gc-style prune.
- [x] Test: removing the last referencing round removes the ref, and row deletion commits first.
- [x] Implement pin, sweep, and the ordered purge path.
- [x] Run tests; expect pass. Commit.
...
**Steps:**
- [x] Test: a parked decision backed by an applicable round serves findings from that round.
- [x] Test: a pre-migration parked run answers approve/fix/skip/abort, notes, and hunk lookup,
      labelled `legacy_snapshot`; an unreviewed run reports `none`.
- [x] Test: legacy rows decode through their own DTO with no enriched field defaulted in.
- [x] Test: `repo_id_for` returns the same value for the same absolute path.
- [x] Implement the record variants, both DTOs, the TUI marker, and the upgrade guidance.
- [x] Run tests; expect pass. Commit.
