...
**Depends-on:** MILE-2
**Commitment:** Planned — the first blocker was due before approval and is still open, so the milestone stays `Planned` even though ROAD-12 has shipped work. `ONEBIN` was scoped to what needs no policy decision, on the `ESCAPE` / `DFAULT` terms.
**Closed:** None — ROAD-12 shipped its first wave but is not closed; the consolidation it names waits on the first blocker.
**Deferred:** None
**Blockers:**
- The exact one-binary command surface and its compatibility period — owner Jayden; due before this milestone is approved for implementation; resolved through feature discovery/design, not here. `ONEBIN` sharpened it and carried recommendations for ratification (`docs/specs/2026-09-09-one-binary-install-coherence/requirements.md` §7): retire the `porch-quality` bin target and keep the library published, which is the only way to remove the cargo output-filename collision cargo already warns "may become a hard error" — two bin targets cannot produce one file name, so no coding session can fix it. Measured installed base at framing: 31 downloads of `porch` across two versions, 22 of `porch-quality`, one reverse dependency (`porch` itself), dogfood not started. A cheaper lever nobody had costed: yank `porch-quality 0.2.1`, or publish the next as library-only, which removes the hazard class with no code — **unverified**, check first whether yanking breaks `cargo install porch --locked` through the packaged lockfile. Also absorbs whether the floor moves in-process, which additionally needs a durable-state fence, because the floor's identity basis today *is* the sibling's artifact hash and it flows into each run's pinned `required_set_digest`.
- Whether native fallback runs automatically or only when explicitly configured — owner Jayden; same due point and route. `ONEBIN` made it executable rather than answering it: installing `porch` puts `porch-quality` on `PATH`, which flips `porch setup --yes` from `agent` to `quality` on a machine that has a coding agent, so porch's own artifact is read as evidence of an operator's choice and the judgment layer is dropped with an empty `warnings` array. Pinned by `tripwire_engine_selection_depends_on_whether_porchs_bindir_is_on_path`. Note the trap for whoever picks this up: `detect_engines` feeds *both* the default and the explicit lookup, so repairing one through detection silently decides the other — which is why `ONEBIN` fixed only the explicit path.
- **New (2026-09-09).** Whether porch should hold an expected identity for the floor and refuse a sibling that does not match — owner Jayden; needs an **ADR**, not a wave. Framing proposed it as ARCH-9/ARCH-12 enforcement; it is not. ARCH-12 forbids substituting an external judgment producer for the floor and ARCH-9 forbids vendoring a third-party CLI as the engine, while `docs/specs/2026-09-02-mandatory-floor/requirements.md:283` states FLOOR-9.2's no-redirect guarantee "covers configuration and environment only; it does not extend to an owner who replaces the installed `porch` or `porch-quality` binaries." Binding that case extends a guarantee a shipped spec bounded on purpose. Two costs to carry into the decision: it needs the audit contract widened first (`AuditReportedVersion` is a strict struct with a required `unavailable` field and `project_descriptor` falls back on whole-descriptor failure, so a new `reported_version` shape blanks every producer's descriptor in every recorded round — a regression into MILE-2's `Closed` read path), and it is dead work under two of the three ways the first blocker can resolve. `ONEBIN` made it observable instead: `porch doctor` now reports the observed artifact identity without judging it, and `tripwire_a_foreign_executable_of_the_right_name_is_accepted_as_the_floor` pins today's answer.
